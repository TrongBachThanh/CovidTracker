...
const configMap = (map, popup, data) => {
  ...
  map.once('load', function () {
    ...
    map.on('mousemove', 'circles', position => {
      const { cases, deaths, recovered, country, province } = position.features[0].properties;
      ...
      popup
        ...
        .setHTML(createPopup(cases, deaths, recovered, country, province, getIso(country)))
        .addTo(map);
    });
...
const createPopup = (cases, deaths, recovered, country, province, countryISO) => {
  const provinceHTML = province !== 'null' ? `<p>District/State: <b>${province}</b></p>` : '';
  ...
  const popUp = `
      ...
      ${provinceHTML}
      <p>Cases: <b>${formatNumber(cases)}</b></p>
      <p>Deaths: <b>${formatNumber(deaths)}</b></p>
      <p>Recovered: <b>${formatNumber(recovered)}</b></p>
      ${countryFlagHTML}
    `;
  return popUp;
};
...
const getIso = country => {
  let countryISO = lookup.byCountry(country)?.iso2.toLowerCase() || lookup.byInternet(country)?.iso2.toLowerCase();
  if (country == 'Burma') countryISO = 'mm';
  if (country == 'Korea, South') countryISO = 'kr';
  if (country == 'Taiwan*') countryISO = 'tw';
  if (country == 'Congo (Brazzaville)' || country == 'Congo (Kinshasa)') countryISO = 'cg';
...
const filterData = (startDate, endDate, cases, deaths, recovered) => {
  let blankArray = [[], [], [], []];
  let [indexDatePicked, casesFiltered, deathsFiltered, recoveredFiltered] = blankArray;
  if (startDate !== '' && endDate !== '') {
    indexDatePicked.push(
      cases?.findIndex(element => element.time == startDate),
      cases?.findIndex(element => element.time == endDate)
    );
    casesFiltered = cases?.filter((element, index) => index >= indexDatePicked[0] && index <= indexDatePicked[1]);
    deathsFiltered = deaths?.filter((element, index) => index >= indexDatePicked[0] && index <= indexDatePicked[1]);
    recoveredFiltered = recovered?.filter(
      (element, index) => index >= indexDatePicked[0] && index <= indexDatePicked[1]
    );
  }
